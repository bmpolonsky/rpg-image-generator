...
export const INITIAL_STATE: AppState = {
  ...
  descriptionModel: 'gemini-3.1-pro-preview',
...
loadState().then((data) => {
  if (data) {
    appStore.update(s => ({
      ...s,
      ...data,
      generateNarrative: data.generateNarrative ?? false,
      mode: data.mode || GenerationMode.BATTLEMAP,
      descriptionModel: data.descriptionModel === 'gemini-3-pro-preview' ? 'gemini-3.1-pro-preview' : (data.descriptionModel || 'gemini-3.1-pro-preview'),
      imageModel: data.imageModel === 'gemini-3-pro-image-preview' ? 'gemini-3.1-flash-image-preview' : (data.imageModel || 'gemini-2.5-flash-image'),
